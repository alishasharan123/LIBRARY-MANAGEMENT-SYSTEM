...
$s_email=$_POST['email'];
$s_pass=$_POST['pass'];
$sel_s="select customer_pass from students where customer_email='$s_email'";
$run_s=mysql_query($sel_s);

if((strcmp($s_email,alisha)==0) AND (strcmp($s_pass,alisha)==0)  )
{
$_SESSION['customer_email']=$s_email;
echo "<script type='text/javascript'>alert('loggingsuccessful')</script>";
echo "<script type='text/javascript'>window.open('pages/librarian_page.php', '_self')</script>";
}
